...
class DFCSEN12MSDataset:
    ...
    def get_patch_ids(self, season, scene_id, sensor=Sensor.s1, ext="tif"):
        """
        ...
        patch_ids = [os.path.splitext(os.path.basename(p))[0] for p in glob(os.path.join(path, f"*.{ext}"))]
    ...
    def get_patch(self, season, scene_id, patch_id, bands, ext="tif"):
        """
        ...
        filename = "{}_{}_p{}.{}".format(season, scene, patch_id, ext)
    ...
    def get_s1_s2_pair(self, season, scene_id, patch_id, s1_bands=S1Bands.ALL, s2_bands=S2Bands.ALL):
        """
            Returns a quadruple of patches. S1, S2, LC and DFC as well as the geo-bounds of the patch. If the number of bands is NONE 
            then a None value will be returned instead of image data
        """
        ...
        bounds = next(filter(None, [bounds1, bounds2]), None)
